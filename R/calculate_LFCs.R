...
#' @examples
#' @importFrom magrittr %>%
calculate_LFCs <- function(ToDcall) {
  ...
  ToDcall@transcripts_LFC_between_timepoints <- ToDcall@transcript_counts_norm %>%
    ...
    LFC_calc_between_timepoints(., ToDcall@timepoints_to_compare)
  ...
  ToDcall@proteome_LFC_between_timepoints <- ToDcall@proteome_norm %>%
    ...
    LFC_calc_between_timepoints(., ToDcall@timepoints_to_compare, is_log_transformed = T)
  ...
  ToDcall@transcripts_LFC_against_0 <- ToDcall@transcript_counts_norm %>%
    tibble::rownames_to_column("gene_id") %>%
    LFC_calc_against_first_timepoint(., ToDcall@timepoints_to_compare)
  ...
  ToDcall@proteome_LFC_against_0 <- ToDcall@proteome_norm %>%
    dplyr::rename(gene_id = 1) %>%
    LFC_calc_against_first_timepoint(., ToDcall@timepoints_to_compare, is_log_transformed = T)
